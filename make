...
echo Compiling Java
javac -d bin/classes -classpath bin/classes -target 1.5 -bootclasspath $AJAR -g  $(find . -name *.java)

echo Compiling Scheme
CLASSPATH=kawa/kawa.jar:$AJAR:bin/classes/ java kawa.repl -d bin/classes -P ${PACKAGE}. --module-static-run --warn-undefined-variable --warn-unknown-member --warn-invoke-unknown-method --warn-as-error -C $(find src -name *.scm)

if [ "$1" == "debug" ] ;
then
#jar cf bin/obfuscated.jar -C kawa/ .
cp kawa/kawa.jar bin/obfuscated.jar
...
echo Signing Jar
jarsigner -keystore $KEYSTORE -storepass $STOREPASS -keypass $KEYPASS bin/$OUT $KEYALIAS
